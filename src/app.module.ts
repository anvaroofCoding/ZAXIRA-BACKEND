...
import { loadBackendEnvFiles } from './config/load-env';

const backendEnvFiles = loadBackendEnvFiles().filter((filePath) =>
  existsSync(filePath),
);
...
const envFilePath = backendEnvFiles.length
  ? [...new Set(backendEnvFiles)]
  : [...new Set(fallbackEnvFiles)];
