...
export class WarehousePricingService {
  ...
  async getUnitPriceMapForStructure(
    structureId: string,
  ...
  private async buildBaseUnitPriceMap(
    structureId: string,
  ): Promise<Map<string, number>> {
    const structureObjectId = new Types.ObjectId(structureId);
      ...
      { $unwind: '$pr' },
      {
        $project: {
          ...
          unitPrice: {
            $let: {
              vars: {
                prItem: { $arrayElemAt: ['$pr.items', '$items.itemIndex'] },
              },
              in: {
                $cond: [
                  { $gt: [{ $ifNull: ['$items.unitPrice', 0] }, 0] },
                  '$items.unitPrice',
                  { $ifNull: ['$$prItem.purchaseAmount', 0] },
                ],
              },
            },
          },
    ...
    const purchaseRows = await this.dispatchModel
      .aggregate(purchasePipeline)
      .exec();
    for (const row of purchaseRows as Array<{
      itemKey: string;
      unitPrice: number;
    }>) {
      if (
    ...
    const transferPipeline: PipelineStage[] = [
      {
        $match: {
          ...
          $or: [
            ...
            { purchaseRequestId: { $exists: false } },
          ],
    ...
    const transferRows = await this.dispatchModel
      .aggregate(transferPipeline)
      .exec();
  ...
  async resolveTransferItemUnitPrice(
    ...
    const sourceMap = await this.getUnitPriceMapForStructure(
  ...
  resolveUnitPriceFromMap(
    ...
    const stored = Math.round(Number(inventoryUnitPrice) || 0);
    if (stored > 0) {
      return stored;
    }

    const fromItemKey = priceMap.get(itemKey) ?? 0;
    if (fromItemKey > 0) {
      return fromItemKey;
    }

    if (name?.trim() || characteristics?.trim()) {
      const computedKey = buildWarehouseItemKey(
        name ?? '',
        characteristics ?? '',
      );
      return priceMap.get(computedKey) ?? 0;
    }
