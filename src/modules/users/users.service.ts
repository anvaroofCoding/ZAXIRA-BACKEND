...
export class UsersService {
  ...
  private parseStructureFromUser(user: UserDocument) {
    const structureRef = user.structureId as
      | { _id: Types.ObjectId; fullName: string; shortName: string }
      | Types.ObjectId
      | undefined
      | null;
  ...
  async createUser(input: CreateUserInput): Promise<UserDocument> {
    ...
    return this.userModel.create({
      ...
      permissions: this.resolvePermissionsForRole(role, input.permissions),
    });
  }

  async createFromDto(dto: CreateUserPayload, createdById?: string) {
  ...
  async remove(id: string, requesterRole: UserRole) {
    const user = await this.findById(id);
