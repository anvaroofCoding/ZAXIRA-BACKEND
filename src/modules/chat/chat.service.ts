  ...
  async getSummary(userId: string, role?: UserRole) {
    ...
    if (isOperator) {
      const supportDocs = await this.chatMessageModel
        .find({ roomType: ChatRoomType.SUPPORT, supportRequesterId: { $ne: null } })
        .sort({ createdAt: -1 })
        .limit(500)
        .exec();
      ...
      support = {
        ...
        lastMessageText: supportLatest ? this.messagePreview(supportLatest) : '',
  ...
  async sendMessage(dto: SendChatMessageDto, userId: string, role?: UserRole) {
    ...
    if (dto.replyToMessageId) {
      ...
      if (
        ...
        throw new BadRequestException('Javob faqat shu chat ichidagi xabarga beriladi');
      }
    ...
    if (server) {
      if (dto.roomType === ChatRoomType.GLOBAL) {
        ...
        server.to('chat:global').emit('chat:summary', { updatedAt: Date.now() });
