...
export class WarehouseDispatchDocumentService {
  ...
  async generatePdf(dispatch: WarehouseDispatchDocument): Promise<Buffer> {
    ...
    return new Promise((resolve, reject) => {
      ...
      doc.text(
        `Holat: ${WAREHOUSE_DISPATCH_STATUS_LABELS[dispatch.status]}`,
      );
      doc.text(
      ...
      sections: [
        {
          children: [
            ...
            new Paragraph({
              children: [new TextRun({ text: 'NAKLADNOY', bold: true, size: 24 })],
              alignment: 'center',
            }),
            new Paragraph({ text: '' }),
            new Paragraph({ text: `Nakladnoy raqami: ${dispatch.dispatchCode}` }),
