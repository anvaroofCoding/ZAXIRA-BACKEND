...
export class WarehouseDispatchesController {
  ...
  countPending(@CurrentUser() user: JwtPayload) {
    return this.warehouseDispatchesService.countPendingReceipt(user.sub, user.role);
  }
  ...
  createTransfer(
    ...
    return this.warehouseDispatchesService.createTransfer(dto, user.sub, user.role);
  }
  ...
  receive(
    ...
    return this.warehouseDispatchesService.receive(id, dto, user.sub, user.role);
  }
}
