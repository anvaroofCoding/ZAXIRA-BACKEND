...
export class StocktakesService {
  ...
  private async assertStructureAccess(
    structureId: string,
    userId: string,
    role?: UserRole,
  ) {
    ...
    if (userStructureId !== structureId) {
      throw new ForbiddenException(
        'Bu tuzilma uchun invertarizatsiya qilishga ruxsat yo‘q',
      );
    }
  }
  ...
  private toPublicLine(line: StocktakeLine) {
    ...
    return {
      ...
      nomenclatureCode: line.nomenclatureCode?.trim() || '',
      bookQuantity,
      countedQuantity,
      excessQuantity,
  ...
  private getExcessLinesFromStocktake(stocktake: StocktakeDocument) {
    return (stocktake.lines ?? []).filter(
      (line) => this.getLineExcess(line) > 0,
    );
  }

  private getShortageLinesFromStocktake(stocktake: StocktakeDocument) {
    return (stocktake.lines ?? []).filter(
      (line) => this.getLineShortage(line) > 0,
    );
  }
  ...
  private async buildGeneralLines(
    structureId: string,
  ...
  private async buildLocationLines(
    ...
    const items = await this.inventoryModel
      ...
      .sort({ name: 1 })
      .select(
        'itemKey name characteristics barcode receiptNomenclatureCode quantity',
      )
      .exec();
    ...
    return items.map((item) => {
      ...
      const itemKey =
        item.itemKey?.trim() || buildWarehouseItemKey(name, characteristics);
      ...
      return {
        ...
        characteristics,
        barcode,
        nomenclatureCode: item.receiptNomenclatureCode?.trim() || '',
  ...
  async create(dto: CreateStocktakeDto, userId: string, role?: UserRole) {
    await this.assertStocktakeCreatePermission(userId, role);
      ...
      items: items.map((item) => {
        ...
        return {
          ...
          summary: {
            ...
            kam: lines.filter(
              (l) =>
                (l.countedQuantity ?? 0) > 0 &&
                (l.countedQuantity ?? 0) < (l.bookQuantity ?? 0),
            ).length,
            ko_p: lines.filter(
              (l) => (l.countedQuantity ?? 0) > (l.bookQuantity ?? 0),
            ).length,
          },
  ...
  private async getEditableStocktake(
    id: string,
    userId: string,
    role?: UserRole,
  ) {
    ...
    if (!isOwner && !isPrivileged) {
      throw new ForbiddenException(
        'Bu invertarizatsiyani tahrirlashga ruxsat yo‘q',
      );
    }

    await this.assertStructureAccess(
      String(stocktake.structureId),
      userId,
      role,
    );
  ...
  async updateLine(
    ...
    if (lineIndex < 0) {
      throw new NotFoundException(
        'Tovar invertarizatsiya ro‘yxatida topilmadi',
      );
    }
  ...
  async scanBarcode(
    ...
    const lineIndex = (stocktake.lines ?? []).findIndex(
      (line) => line.barcode === value,
    );

    if (lineIndex < 0) {
      const computedIndex = (stocktake.lines ?? []).findIndex((line) => {
        const computed = computeWarehouseBarcode(
          line.name,
      ...
      if (computedIndex < 0) {
        throw new NotFoundException(
          'Barcode invertarizatsiya ro‘yxatida topilmadi',
        );
      }
  ...
  private async setLineInventoryTarget(
    ...
    await this.setGeneralInventoryTarget(
      String(stocktake.structureId),
      line,
      targetQuantity,
    );
  ...
  private async applyLocationInventory(stocktake: StocktakeDocument) {
    for (const line of stocktake.lines ?? []) {
      const target =
        (line.countedQuantity ?? 0) - (line.excessDeductQuantity ?? 0);
  ...
  private async setGeneralInventoryTarget(
    ...
    const currentTotal = inventories.reduce(
    ...
    return {
      items: items.map((item) => {
        ...
        const pendingShortageCount = shortageLines.filter((line) => {
          const shortage =
            (line.bookQuantity ?? 0) - (line.countedQuantity ?? 0);
  ...
  async getManagementDetail(id: string, userId: string, role?: UserRole) {
    ...
    if (stocktake.status !== StocktakeStatus.COMPLETED) {
      throw new BadRequestException(
        'Faqat yakunlangan invertarizatsiyalar boshqariladi',
      );
    }
    ...
    const excessLines = this.getExcessLinesFromStocktake(stocktake).map(
      (line) => this.toPublicLine(line),
    );
    const shortageLines = this.getShortageLinesFromStocktake(stocktake).map(
      (line) => this.toPublicLine(line),
    );

    if (!excessLines.length && !shortageLines.length) {
      throw new BadRequestException(
        'Bu partiyada ko‘p yoki kam sanalgan tovar yo‘q',
      );
    }
  ...
  async applyExcessAdjustments(
    ...
    const lineByKey = new Map(
      (stocktake.lines ?? []).map((line) => [line.lineKey, line]),
    );
