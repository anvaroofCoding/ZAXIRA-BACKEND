...
export class StocktakesController {
  ...
  @Get('management')
  listForManagement(@Query() query: QueryStocktakesDto, @CurrentUser() user: JwtPayload) {
  ...
  @Get(':id/management')
  getManagementDetail(@Param('id', ParseMongoIdPipe) id: string, @CurrentUser() user: JwtPayload) {
  ...
  applyExcessAdjustments(
    ...
    return this.stocktakesService.applyExcessAdjustments(id, dto, user.sub, user.role);
  }
  ...
  @Get(':id')
  findById(@Param('id', ParseMongoIdPipe) id: string, @CurrentUser() user: JwtPayload) {
  ...
  @Post(':id/complete')
  complete(@Param('id', ParseMongoIdPipe) id: string, @CurrentUser() user: JwtPayload) {
    return this.stocktakesService.complete(id, user.sub, user.role);
  }

  @Post(':id/cancel')
  cancel(@Param('id', ParseMongoIdPipe) id: string, @CurrentUser() user: JwtPayload) {
